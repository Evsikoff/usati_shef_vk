...
(function(o) {
    const u = class {
        ...
        initFrom(r, e) {
            ...
            for (let t = 0; t < r.length; ++t) {
                ...
                if (l.reinitialize(n), !e) {
                    i < a._variablesArray.length ? a._variablesArray[i] = l : a._variablesArray.push(l), ++i;
                    const s = u._deletedVars.indexOf(n.name);
                    s !== -1 && (u._deletedVars[s] = void 0)
                }
            }
            if (!e) {
                this._variablesArray.length = i;
                for (let t = 0, n = u._deletedVars.length; t < n; ++t) {
                    const l = u._deletedVars[t];
        ...
        rebuildIndexFrom(r) {
            ...
            for (const e of r)
                if (e.name) {
                    const a = this._variables.get(e.name);
                    this._variablesArray.push(a)
                }
        }
        _declare(r, e) {
        ...
        add(r, e) {
            const a = this._variables.get(r);
            if (this._variables.put(r, e), a) {
                const i = this._variablesArray.indexOf(a);
        ...
        hasVariable(r) {
            const e = this._variablesArray.find(a => a === r);
            return !!e && !e.isUndefinedInContainer()
        }
        ...
        getNetworkSyncData(r) {
            const e = r.playerNumber,
                ...
                i = [],
                t = [];
            return this._variables.keys(t), t.forEach(n => {
                const l = this._variables.get(n),
                    s = l.getPlayerOwnership();
                if (l.isUndefinedInContainer() || s === null || e !== void 0 && s === 0 && !a || s !== 0 && e !== s) return;
                const c = l.getType(),
                    b = c === "structure" || c === "array" ? "" : l.getValue();
                i.push({
                    name: n,
                    value: b,
                    type: c,
                    children: this.getStructureNetworkSyncData(l),
                    owner: s
                })
            }), i
        }
        getStructureNetworkSyncData(r) {
            if (r.getType() === "array") {
                const e = [];
                return r.getAllChildrenArray().forEach(a => {
                    const i = a.getType(),
                        t = i === "structure" || i === "array" ? "" : a.getValue(),
                        n = a.getPlayerOwnership();
                    a.isUndefinedInContainer() || n === null || e.push({
                        name: "",
                        value: t,
                        type: i,
            ...
            if (r.getType() === "structure") {
                ...
                return Object.entries(e).forEach(([i, t]) => {
                    const n = t.getType(),
                        l = n === "structure" || n === "array" ? "" : t.getValue(),
                        s = t.getPlayerOwnership();
                    t.isUndefinedInContainer() || s === null || a.push({
                        name: i,
                        value: l,
                        type: n,
        ...
        updateFromNetworkSyncData(r) {
            ...
            for (let a = 0; a < r.length; ++a) {
                const i = r[a],
                    t = e._getVariableDataFromNetworkSyncData(i),
                    n = t.name;
                if (!n) continue;
                const l = e.get(n),
                    s = i.owner,
                    c = o.multiplayer.getCurrentPlayerNumber(),
                    b = l.getPlayerOwnership();
                if (c === b) {
                    console.info(`Variable ${n} is owned by us ${o.multiplayer.playerNumber}, ignoring update message from ${s}.`);
                    return
                }
                s !== b && (console.info(`Variable ${n} is owned by ${b} on our game, changing ownership to ${s} as part of the update event.`), l.setPlayerOwnership(s)), l.reinitialize(t)
            }
        }
    ...
    let d = u;
    d._deletedVars = [], d.badVariablesContainer = {
